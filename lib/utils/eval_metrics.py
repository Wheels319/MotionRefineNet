...
def evaluate_smoothnet_3D(model, test_dataloader, device, dataset_name,estimator,cfg):
    ...
    # denoise_mpjpe_i = torch.empty((0)).to(device)
    # denoise_pampjpe_i = torch.empty((0)).to(device)
    # denoise_accel_i = torch.empty((0)).to(device)
    ...
    for i, data in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            data_pred=data_pred.permute(0,2,1)
            start_time=time.time()
            # denoised_pos2, denoised_pos = model(
            #     data_pred)
            denoised_pos = model(
                data_pred)
            inference_time["smoothnet"]+=time.time()-start_time
            data_pred=data_pred.permute(0,2,1)
            denoised_pos=denoised_pos.permute(0,2,1)
            #denoised_pos2 = denoised_pos2.permute(0, 2, 1)


        denoised_pos = slide_window_to_sequence(denoised_pos,cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,cfg.MODEL.SLIDE_WINDOW_SIZE)
        # denoised_pos2 = slide_window_to_sequence(denoised_pos2, cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,
        ...
        #denoised_pos2 = denoised_pos2.reshape(frame_num, -1, 3)
        ...
        if cfg.EVALUATE.ROOT_RELATIVE:
            denoised_pos = denoised_pos - denoised_pos[:,
                                                          keypoint_root, :].mean(
                                                              axis=1).reshape(
                                                                  -1, 1, 3)
            # denoised_pos2 = denoised_pos2 - denoised_pos2[:,
            #                               keypoint_root, :].mean(
            #     axis=1).reshape(
            #     -1, 1, 3)
        ...
        # denoise_mpjpe_i = torch.cat(
        #     (denoise_mpjpe_i, calculate_mpjpe(denoised_pos2, data_gt)), dim=0)
        # denoise_pampjpe_i = torch.cat(
        #     (denoise_pampjpe_i, calculate_pampjpe(denoised_pos2, data_gt)), dim=0)
        # denoise_accel_i = torch.cat(
        #     (denoise_accel_i, calculate_accel_error(denoised_pos2, data_gt)), dim=0)
