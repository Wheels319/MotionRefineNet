    ...
    def train(self):
        ...
        for iter_index in range(training_iter):
            for data_index in range(len(train_dataloader)):
                ...
                # denoised_pos1,denoised_pos2 = self.model(
                #     data_pred)
                denoised_pos1= self.model(data_pred)
                denoised_pos1=denoised_pos1.permute(0,2,1)
                #denoised_pos2 = denoised_pos2.permute(0, 2, 1)

                timer['forward'] = time.time() - start
                start = time.time()

                #loss_total = self.loss_init(denoised_pos1, data_gt)+self.loss(denoised_pos2, data_gt)
                loss_total = self.loss(denoised_pos1, data_gt)
